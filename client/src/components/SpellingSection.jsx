        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
          ...
          <div className="space-y-6">
            ...
            <h2 className="text-4xl md:text-5xl font-black text-white leading-[1.1]">
              {isRu
                ? <>Как бы вы ни написали,<br />Omogle — это то,<br />что вы искали.</>
                : <>However you spell it,<br />Omogle is the app.</>}
            </h2>
            <p className="text-gray-400 text-base leading-relaxed max-w-md">
              {isRu
                ? "Люди попадают сюда через разные варианты написания. Omogle — это название приложения, и мы подключаем все популярные запросы, чтобы те, кто набирает omogle, ommogle или ommoggle omoggle, всегда попадали куда нужно."
                : "People arrive here through a few spellings. Omogle is the name of the app, and we keep the common searches connected so visitors who type omogle, ommogle, or ommoggle omoggle still get where they meant to go."}
            </p>
          </div>

          {/* Right side — spelling cards */}
          <div className="grid grid-cols-2 gap-3">
            {spellings.map((item) => (
              <div
                key={item.term}
                className="group bg-white/[0.03] border border-white/10 rounded-2xl p-5 hover:border-cyber-neon/30 hover:bg-white/[0.05] transition-all duration-300 cursor-default"
              >
