      ...
      <div className="min-h-screen flex items-center justify-center p-4 relative overflow-hidden">
        ...
        {user && (
          <div className="absolute top-6 right-6 z-20 flex items-center gap-4 bg-black/60 border border-cyber-border px-4 py-2 rounded-lg backdrop-blur-md shadow-[0_0_15px_rgba(0,255,157,0.1)]">
            {user.avatarUrl ? (
              <img src={user.avatarUrl.startsWith('data:') ? user.avatarUrl : `${SOCKET_URL}${user.avatarUrl}`} alt="avatar" className="w-6 h-6 rounded-full object-cover border border-cyber-neon" />
            ) : (
          ...
          <div className="absolute top-4 right-4 z-20 flex flex-col items-end">
             <div className="flex items-center gap-2 mb-1">
                ...
                <div className="w-6 h-6 rounded-full bg-[#5d8b4e] flex items-center justify-center text-black font-bold text-xs shadow-md">
                   {user?.avatarUrl ? <img src={user.avatarUrl.startsWith('data:') ? user.avatarUrl : `${SOCKET_URL}${user.avatarUrl}`} className="w-full h-full rounded-full object-cover"/> : "L"}
                </div>
             </div>
